...
def train(data):
    ...
    ensemble=VotingClassifier(estimators=[('SVM', clf_svm), ('naiveBayes', clf_naiveBayes), ('KNN', clf_kNeighbors), ("decisionTree",clf_decisionTreee)], 
                       voting='hard', weights=[1,1,1,1]).fit(X,Y)
    return ensemble

def predict(data, model, to_csv=False, targets=None, from_file=False):
    if (from_file):
        data = pd.read_csv(data)
        content_data = vectorizer.transform(data["article"].values.astype('U')).toarray()
    else:
